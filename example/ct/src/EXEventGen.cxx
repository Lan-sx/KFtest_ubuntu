...
THelicalTrack EXEventGen::GenerateHelix(Double_t pt)
{
    ...
    //Double_t fi0 = __FI0__ + TMath::Pi()/3.;
    Double_t cpa = 1. / pt;
    Double_t dz  = __DZ__;
    ...
    //std::cout<<"================> Generate Helical B = "<<b<<std::endl;
...
void EXEventGen::Swim(THelicalTrack &heltrk)
{
    ...
    for (Int_t lyr = 0; lyr < nlayers && lyr >= 0; lyr += dlyr) {
        ...
        if (!ms.CalcXingPointWith(heltrk,xx,dfi,1)) {
            std::printf("==========Debug Br1 %d-th layer dfi=%.3f\n",lyr,dfi);
            if (is1stloop && fHitBufPtr->GetEntries() > 3) {
                is1stloop = kFALSE;
                dlyr      = -1;
                dfi       = dfis;
                std::printf("==========Debug Br1_1 %d-th layer dfi=%.3f\n",lyr,dfi);
                continue;
            } else break;
        }
        // std::cout<<"========> "<<xx.x()<<","<<xx.y()<<","<<xx.z()<<std::endl;
        // should use the material behind the surface since dfi is measured 
        // from the last point to the current surface
        Bool_t   dir    = dlyr < 0 ? kTRUE : kFALSE;

        if (fCradlePtr->IsMSOn()) {
            std::printf("==========Debug Br2 %d-th layer dfi=%.3f\n",lyr,dfi);
            TKalMatrix Qms(5,5);
        ...
        //std::printf("==========Debug %d-th layer dfi=%.3f\n",lyr,dfi);
        ...
        //std::cout<<"===> "<<lyr<<"\t"<<ms.GetEnergyLoss(dir, heltrk, dfis)<<std::endl; // energy loss
        
        av(2,0) += ms.GetEnergyLoss(dir, heltrk, dfis); // energy loss
        heltrk.SetTo(av, heltrk.GetPivot());

        if (ms.IsActive()) {
            // std::printf("==========Br3 Debug %d-th layer dfi=%.3f\n",lyr,dfi);
            ms.ProcessHit(xx, *fHitBufPtr);
        }
    }
}
